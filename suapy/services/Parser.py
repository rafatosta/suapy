...
def extrair_nome_e_matricula(string):
    ...
    if match:
        ...
        nome_secundario = match.group(2).strip() if match.group(2) else None
...
def extrair_dados_bancarios(string):
    ...
    dados_bancarios = {'Banco': None, 'Agência': None, 'Conta': None, 'Operação': None}
    
    # Processa as linhas
    for i, linha in enumerate(linhas):
        if "Banco:" in linha:
            # Captura o valor da linha seguinte
            dados_bancarios['Banco'] = linhas[i + 1].strip() if i + 1 < len(linhas) else None
        elif "Agência:" in linha:
            # Captura o valor da linha seguinte
            dados_bancarios['Agência'] = linhas[i + 1].strip() if i + 1 < len(linhas) else None
        elif "Conta:" in linha:
            # Captura o valor da linha seguinte
            dados_bancarios['Conta'] = linhas[i + 1].strip() if i + 1 < len(linhas) else None
        elif "Operação:" in linha:
            # Captura o valor da linha seguinte
            dados_bancarios['Operação'] = linhas[i + 1].strip() if i + 1 < len(linhas) else None
